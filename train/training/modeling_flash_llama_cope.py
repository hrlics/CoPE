...
class LlamaRotaryEmbedding(nn.Module):
    def __init__(
        ...
        assert clip_low_n >=0 , "clip_low_n must be non-negative"
        assert type(clip_low_n) == int, "clip_low_n must be an integer"
        self.clip_low_n = clip_low_n
        ...
        if self.clip_low_n > 0:
            ...
            if clip > 0:
                ...
                # NOTE: HardClip implementation
                # start_idx = freq_dim - clip
                # inv_freq = inv_freq.clone()
                # inv_freq[start_idx:] = 0.0
